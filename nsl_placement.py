...
def nsl_placement(nslr, substrate):
    ...
    ranked_nodes_cpu = sort_nodes(nodes,
                                  "node_potential")  # ranked list of nodes by potential considering cpu and conections
        ...
        for n in ranked_nodes_cpu:
            ...
            if v["cpu"] <= n["cpu"] and v["type"] == n["type"] and n["id"] not in already:
                ...
                if ranked_nodes_cpu.index(n) == len(
                        ranked_nodes_cpu) - 1:  # slice rejection only when no node has enough resources
                    rejected = True
                    break
                    ################### ------------- #################
